...
Namespace Models
    Public Class CURD
            ...
            Try
                ...
                Using SqlCon As New SqlConnection(ConnectionStringToUse)
                    ...
                    For Each member As PropertyInfo In InputProps.GetProperties
                        ...
                        ElseIf member.Name.ToLower = "updatedon" Then
                            UpdateFields.Add(member.Name & "=@" & member.Name)
                            UpdateParams.Add("@" & member.Name)
        ...
        Public Shared Function DeleteDataComplexCondition(ByVal DQ As ComplexDeleteQuery) As Boolean
            ...
            End Try
        End Function
        Public Shared Function CustomComplextUpdateDataQuery(ByVal CUQ As ComplexCustomUpdateQuery) As Boolean
            Try
                Dim ConnectionStringToUse As String = String.Empty
                If CUQ._DB = "SAP" Then
                    ConnectionStringToUse = SAPConString
                ...
                Using SqlCon As New SqlConnection(ConnectionStringToUse)
                    ...
                    SqlCon.Open()
                    Dim TableName As String = CUQ._FilterTable(0).GetType().Name
                    ...
                    Dim props As Type = CUQ._FilterTable(0).GetType
                    Dim j As Integer = 0
                    For Each Obj In CUQ._FilterTable
                        For Each member As PropertyInfo In props.GetProperties
                            If j = 0 Then
                                If Not IsNothing(member.GetValue(CUQ._FilterTable(j), Nothing)) Then
                                    FilterParams.Add("@" & member.Name)
                                    FilterParamsValues.Add(member.GetValue(CUQ._FilterTable(j), Nothing).ToString)
                                End If
                            Else
                                If Not IsNothing(member.GetValue(CUQ._FilterTable(j), Nothing)) Then
                                    FilterParams.Add("@" & member.Name & j)
                    ...
                    Qry = Qry & CUQ._UpdateQuery

                    If CUQ._HasWhereConditions = True Then
                        Qry = Qry & " WHERE "


                        If CUQ._Conditions.Count > 0 Then
                            Dim ConditionsCount As Integer = 0
                            Dim pair As KeyValuePair(Of String, List(Of String))
                            For Each pair In CUQ._Conditions
                                ' Display Key and Value.
                                If ConditionsCount > 0 And CUQ._HasInBetweenConditions = True Then
                                    Qry = Qry & CUQ._InBetweenCondition
                                End If
